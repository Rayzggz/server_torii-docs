...
`Torii-Feature-Control` is a trusted internal request header from Nginx to Server Torii. It must be generated by the reverse proxy; never accept or forward a client-supplied value, and never set it from `$http_torii_feature_control`. Keep Server Torii's port `25555` private and never expose it to the public Internet.
:::

Nginx inherits `proxy_set_header` directives from the previous configuration level only when the current level has no `proxy_set_header` directives of its own. Therefore, do not configure `Torii-Feature-Control` only at the `http` or `server` level. Set it explicitly in every Torii location, as shown below, or put the same directive in an `include` file used by every Torii location.
...
> This is a trusted internal control header. It must only be generated by the reverse proxy and must never be controlled by an external client.
...
Because `'0'` can disable a feature that is enabled in the configuration file, protecting this header is a mandatory security requirement, not an optional recommendation. Never derive it from `$http_torii_feature_control` or any other client-controlled value.
